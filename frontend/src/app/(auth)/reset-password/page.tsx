...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        <p style={{ color: '#f87171', marginBottom: '24px' }}>Invalid or missing password reset token.</p>
        <Link href="/forgot-password" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: '#818cf8', fontSize: '14px', textDecoration: 'none', fontWeight: '500' }}>
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: '16px 0' }}>
        <div style={{ width: '64px', height: '64px', background: 'rgba(34,197,94,0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: '#22c55e' }}>
          <CheckCircle2 size={32} />
        </div>
        <h2 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '12px' }}>Password reset successfully</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Redirecting to login...</p>
      </div>
    );
  }
  ...
  return (
    <>
      <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#f1f5f9', marginBottom: '8px' }}>Set new password</h1>
      <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '24px' }}>Your new password must be different from previously used passwords.</p>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input type="password" required className="crm-input" style={{ paddingLeft: '40px' }} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" />
          </div>
        </div>

        <div style={{ marginBottom: '24px' }}>
          <label style={{ display: 'block', fontSize: '13px', fontWeight: '500', color: '#94a3b8', marginBottom: '6px' }}>Confirm New Password</label>
          <div style={{ position: 'relative' }}>
            <Lock size={18} style={{ position: 'absolute', left: '12px', top: '10px', color: '#64748b' }} />
            <input type="password" required className="crm-input" style={{ paddingLeft: '40px' }} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" />
          </div>
        </div>
